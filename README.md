...
* 建立简单索引:
1. 建立索引:db.randomInfo.ensureIndex({username:1})
2. 查看索引:db.randomInfo.getIndexes()

* mongodb限制每个collection最多64个索引

* 索引会增加了内存和硬盘的消耗,使用索引的注意事项:
1. 数据超过万条,性能提升明显
2. 查询数据低于表数据30%(如性别,选择少的选项)
3. 数字索引比字符串索引快得多
4. 把经常查询的数据做成内嵌数据(对象型),然后集体进行索引

## 12.索引：复合索引

* 索引查询的默认顺序是按照数据库内索引建立顺序的,hint可以指定索引查询,常用于数字索引
* 索引删除 dropIndex()传入的参数是索引的name值,而不是key值randomNum0

## 13.索引：全文索引

* 需要在大篇幅的文章中搜索关键词,可以使用全文索引:
* 建立索引:db.info.ensureIndex({contextInfo:'text'})
